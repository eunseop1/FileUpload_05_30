...
<%
	...
	try{
		...
		in = new FileInputStream(file); //입력 스트림을 만든다
		// 파일 다운로드 헤더 지정
		response.reset() ;
		...
		if(file!=null){ // 파일이 존재 한다면
			...
			if(client.indexOf("Trident")==-1){ // IE 인터넷 익스플로러가 아니라면
				of = new String(of.getBytes("utf-8"),"iso-8859-1");
			}else{ // IE ext 인터넷 익스플로러가 맞다면 +를 모두 빈칸으로 바꿔라
				of = URLEncoder.encode(of, "UTF-8" ).replaceAll("\\+","%20" );
			}
			response.setHeader("Content-Disposition", "attachment; filename=\""	+ of + "\"");//원본 이름 지정
			...
			os = response.getOutputStream();//reponse 객체를 통해 출력스트림을 얻는다
			
			// 복사
			byte b[] = new byte[(int)file.length()]; // 파일 크기만큼 배열선언
			int leng = 0;
			while( (leng = in.read(b)) > 0 ){ // in에서 읽기
				os.write(b,0,leng); // 쓰기
				os.flush();
			}
		}else{//거의 실행될일은 없지만 혹시나 해서
